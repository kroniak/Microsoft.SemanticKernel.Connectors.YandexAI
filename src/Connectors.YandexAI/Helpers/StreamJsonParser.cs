...
/// <summary>
///     Internal class for parsing a stream of text which contains a series of discrete JSON strings into en enumerable
...
///     If you need a specialized SSE parser, use <see cref="Microsoft.SemanticKernel.Text.SseJsonParser" /> instead.<br />
...
internal sealed class StreamJsonParser
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (await chunkParser.ExtractNextChunkAsync(validateJson, cancellationToken).ConfigureAwait(false) is
               { } json)
            yield return json;
    }
    ...
    private sealed class ChunkParser
    {
            ...
            CancellationToken cancellationToken)
        {
                   ...
                   ).ConfigureAwait(false)) is not null || _lastLine is not null)
            {
                ...
                if (ProcessLineUntilCompleteJson(line!)) return GetJsonString(validateJson);
        ...
        private bool ProcessLineUntilCompleteJson(string line)
        {
            for (var i = 0; i < line.Length; i++)
            {
                ...
                if (IsEscapedCharacterInsideQuotes()) continue;
        ...
        private string GetJsonString(bool validateJson)
        {
            ...
            if (validateJson) _ = JsonNode.Parse(json);
        ...
        {
            if (_insideQuotes) return;

            switch (_currentCharacter)
            {
                case '{':
                    if (++_bracketsCount == 1) _startBracketIndex = index;

                    break;
                case '}':
                    if (--_bracketsCount < 0) throw new InvalidOperationException("Invalid JSON in stream.");

                    if (_bracketsCount == 0) MarkJsonAsComplete();
        ...
        {
            if (this is { _currentCharacter: '\"', _isEscaping: false }) _insideQuotes = !_insideQuotes;
        }
